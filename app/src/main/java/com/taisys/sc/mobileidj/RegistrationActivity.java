...
public class RegistrationActivity extends Activity {
    ...
    private void generateRsaKeyPair(){
        ...
        if (res[0]!=null && res[0].equals("-15")) {    //key不存在，建立key
        ...
        }else if(res[1].indexOf("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")==0) {
            Log.i("MobileIdRegistration", "Sunny: the public key is incorrect, will generate a new one!");
            ...
            begintime = System.currentTimeMillis() - begintime;
            if (resString != null && resString.equals(Card.RES_OK)) {
                Log.d("MobileIdRegistration", "Sunny: time:" + begintime + "ms, " + "Gen key pair OK!");
            } else {
                if (mCard!=null){
                    mCard.CloseSEService();
                }
                Log.e("MobileIdRegistration", "Sunny: time:" + begintime + "ms, " + "Gen key pair Failed!");
                utility.showMessage(myContext, getString(R.string.msgUnableToGenerateRsaKeyPair) + ", error code=" + resString);
                disWaiting();
                return;
            }
